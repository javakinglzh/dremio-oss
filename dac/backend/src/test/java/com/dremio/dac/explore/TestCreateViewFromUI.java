...
public class TestCreateViewFromUI extends DremioTest {
  ...
  public void testCreateVersionedViewVerify()
    ...
    doReturn(storagePlugin).when(catalog).getSource(anyString());
    doReturn(false).when(storagePlugin).isWrapperFor(SupportsMutatingViews.class);
  ...
  public void testCreateViewWithSupportsMutatingViewsVerify()
    ...
    doReturn(false).when(datasetVersionResource).isVersionedPlugin(SOURCE_DATASET_PATH, catalog);
    doReturn(storagePlugin).when(catalog).getSource(anyString());
    doReturn(true).when(storagePlugin).isWrapperFor(SupportsMutatingViews.class);
    doReturn(true).when(storagePlugin).isWrapperFor(SupportsIcebergRestApi.class);
    doReturn(virtualDatasetUI).when(datasetService).getVersion(datasetPath, version, false);
    doNothing()
        .when(datasetService)
  ...
  @Test
  public void testCreateNonVersionedViewFromExistingViewVerify()
      throws IOException,
          UserNotFoundException,
          NamespaceException,
          CatalogEntityAlreadyExistsException,
    ...
    doReturn(storagePlugin).when(catalog).getSource(anyString());
    doReturn(false).when(storagePlugin).isWrapperFor(SupportsMutatingViews.class);
    doReturn(false).when(storagePlugin).isWrapperFor(SupportsIcebergRestApi.class);
    doReturn(virtualDatasetUI).when(datasetService).getVersion(datasetPath, version, false);
    doReturn(datasetUI).when(datasetVersionResource).newDataset(virtualDatasetUI, null);
    datasetVersionResource.saveAsDataSet(SOURCE_DATASET_PATH, null, null);
    verify(datasetService, times(1)).put(virtualDatasetUI);
  }

  @Test
  public void testCreateNonVersionedViewFromExistingViewWithSameNameShouldThrow()
      throws IOException, UserNotFoundException, NamespaceException {
    setup();
    assertThatThrownBy(() -> datasetVersionResource.saveAsDataSet(datasetPath, null, null))
        .isInstanceOf(UserException.class)
