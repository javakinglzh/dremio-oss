...
import HelpSectionMixin from "dyn-load/pages/JobDetailsPage/components/HelpSection/HelpSectionMixin";
...
export class HelpSection extends PureComponent {
  static propTypes = {
    ...
    asyncUpdatesEnabled: PropTypes.bool,
  ...
  handleDownload = () => {
    if (
      this.props.asyncUpdatesEnabled &&
      this.props.jobDetails?.get("isComplete") &&
      !this.props.jobDetails.get("isProfileUpdateComplete")
    ) {
      this.props.addNotification(
        this.props.intl.formatMessage({
          id: "Job.Profile.Download.Incomplete",
        }),
        "warning",
        5,
      );
    ...
    return (
      <div className={helpSectionClassNames}>
        ...
        <div className="helpSection__quoteWrapper">
          ...
          <div className="helpSection__buttons">{buttons.toArray()}</div>
        </div>
      </div>
    );
  }
