...
import com.dremio.plan.serialization.RexIndirection;
...
public final class RexSerializer {
  ...
  public PRexNode toProto(RexNode node) {
    return new RexTraverser(new ArrayList<>()).toProtoIndirection(node);
  }

  public PRexWindowBound toProto(RexWindowBound rexWindowBound) {
    return new RexTraverser(new ArrayList<>()).toProtoIndirection(rexWindowBound);
  }

  private class RexTraverser implements RexVisitor<PRexNode> {

    private final List<RexNode> pojoList;

    public RexTraverser(final List<RexNode> rexNodeList) {
      pojoList = rexNodeList;
    }

    public PRexNode toProtoIndirection(RexNode node) {
      List<PRexNode> protoList = new ArrayList<>();
      PRexNode proto = toProtoInternal(node);
      for (int i = 0; i < pojoList.size(); i++) {
        protoList.add(toProtoInternal(pojoList.get(i)));
      }
      if (pojoList.isEmpty()) {
        return proto;
      } else {
        return PRexNode.newBuilder()
            .setIndirection(
                RexIndirection.newBuilder().setRex(proto).addAllChildren(protoList).build())
            .build();
      }
    }

    public PRexWindowBound toProtoIndirection(RexWindowBound node) {
      List<PRexNode> protoList = new ArrayList<>();
      PRexWindowBound proto = toProto(node);
      ...
      if (pojoList.isEmpty()) {
        ...
        return proto.toBuilder()
            .setBounded(
                proto.getBounded().toBuilder()
                    .setOffset(
                        PRexNode.newBuilder()
                            .setIndirection(
                                RexIndirection.newBuilder()
                                    .setRex(proto.getBounded().getOffset())
                                    .addAllChildren(protoList)
                                    .build())
                            .build()))
            .build();
      }
    ...
    private PRexNode toProtoInternal(RexNode node) {
      if (node instanceof RexOver) {
        return visitOver((RexOver) node);
      } else if (node instanceof RexFieldAccess) {
        return visitFieldAccess((RexFieldAccess) node);
      } else if (node instanceof RexCorrelVariable) {
        return visitCorrelVariable((RexCorrelVariable) node);
      } else if (node instanceof RexDynamicParam) {
        return visitDynamicParam((RexDynamicParam) node);
      } else if (node instanceof RexRangeRef) {
        return visitRangeRef((RexRangeRef) node);
      } else if (node instanceof RexSubQuery) {
        return visitSubQuery((RexSubQuery) node);
      } else if (node instanceof RexTableInputRef) {
        return visitTableInputRef((RexTableInputRef) node);
      } else if (node instanceof RexPatternFieldRef) {
        return visitPatternFieldRef((RexPatternFieldRef) node);
      } else if (node instanceof RexInputRef) {
        return visitInputRef((RexInputRef) node);
      } else if (node instanceof RexLocalRef) {
        return visitLocalRef((RexLocalRef) node);
      ...
      throw new UnsupportedOperationException("Unhandled serialization for " + node.getClass());
    }
    ...
    public PRexNode visitCall(RexCall call) {
      ...
      PRexCall.Builder builder =
          PRexCall.newBuilder()
              .addAllOperands(
                  call.getOperands().stream().map(this::toProto).collect(Collectors.toList()))
    ...
    public PRexNode toProto(RexNode node) {
      int index = pojoList.size();
      pojoList.add(node);
      return PRexNode.newBuilder().setReference(index).build();
    }
    ...
    public PRexNode visitOver(RexOver over) {
              ...
              .map(
                  rexFieldCollation ->
                      PRexFieldCollation.newBuilder()
                          .setLeft(toProto(rexFieldCollation.left))
                          .addAllRight(
      ...
      builder
          ...
          .addAllPartitionKeys(
              w.partitionKeys.stream().map(this::toProto).collect(Collectors.toList()))
          .setIsRows(w.isRows());
      ...
      return PRexNode.newBuilder()
          .setRexOver(
              PRexOver.newBuilder()
                  .addAllOperands(
                      over.operands.stream().map(this::toProto).collect(Collectors.toList()))
    ...
    public PRexWindowBound toProto(RexWindowBound bound) {
      ...
      PRexNode offset = bound.getOffset() == null ? null : toProtoInternal(bound.getOffset());
    ...
    public PRexNode visitFieldAccess(RexFieldAccess fieldAccess) {
      PRexFieldAccess pRexFieldAccess =
          PRexFieldAccess.newBuilder()
              .setExpression(toProto(fieldAccess.getReferenceExpr()))
    ...
    private PRexVariable visitRexVariable(RexVariable rexVariable) {
      return PRexVariable.newBuilder()
          .setType(typeSerializer.toProto(rexVariable.getType()))
    ...
    public PRexNode visitRangeRef(RexRangeRef rangeRef) {
      PRexRangeRef pRexRangeRef =
          PRexRangeRef.newBuilder()
              .setType(typeSerializer.toProto(rangeRef.getType()))
    ...
    public PRexNode visitSubQuery(RexSubQuery subQuery) {
      ...
      PRexSubQuery pRexSubQuery =
          PRexSubQuery.newBuilder()
              ...
              .addAllOperands(
                  subQuery.getOperands().stream().map(this::toProto).collect(Collectors.toList()))
      ...
      return PRexNode.newBuilder()
          .setRexPatternFieldRef(
              PRexPatternFieldRef.newBuilder()
                  ...
                  .setRexInputRef(
                      PRexInputRef.newBuilder()
                          .setDataType(typeSerializer.toProto(fieldRef.getType()))
