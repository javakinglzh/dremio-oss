...
/** Test profile store where storage is on dist store */
public class TestDistProfileStore extends BaseTestServer {
  private ProfileStore profileStore;
  private Provider<ProfileDistStoreConfig> profileDistStoreConfigProvider;

  @Before
  public void setup() throws Exception {
    profileDistStoreConfigProvider =
        l(LocalJobTelemetryServer.class).getProfileDistStoreConfigProvider();
  ...
  @Test
  public void testDeleteBackgroundCleaner() {
    final JobId jobId = JobsServiceUtil.getExternalIdAsJobId(ExternalIdHelper.generateExternalId());
    final AttemptId attemptId = new AttemptId(JobsServiceUtil.getJobIdAsExternalId(jobId), 0);
    final UserBitShared.QueryId queryId = attemptId.toQueryId();
    ...
    // delete and verify.
    DistProfileCleanerImpl.deleteProfiles(
        0,
